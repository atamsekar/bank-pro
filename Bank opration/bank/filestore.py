...
def cusaccountcheck():
        ...
        while name not in cusnames and len(name)<3:
                name=input(r"Please type in your name for this new bank account\n")
                if name not in cusnames:
                        cusnames.append(name)
                        filewrite(cusnames)
                        break
                print("Sorry, that user name is already in use")
                ans=input(r"Are you already a member at this bank? (y/n)\n")
                if ans.lower()=='y':
                        oldcuscheck()
                else:
                        cusaccountcheck()
        ...
        while len(pin)<4:
                pin=input(r"Please assign a password to this account, pin should be at least 5 characters\n")
                if len(pin)>4:
...
def oldcuscheck():
        ...
        while name not in cusnames:
                ...
                if name in cusnames:
                        ...
                        again=input(r"would like to type in your name again? (y/n)")
                        if again.lower()=='y':
                                oldcuscheck()
                        else:
